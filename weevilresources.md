---
layout: default
title: Weevil Resources
has_children: true
nav_order: 4
---

# Weevil Resources
{: .no_toc }

Curculionoidea resources. 
{: .fs-5 .fw-200 }

![001653181_lateral](https://github.com/user-attachments/assets/555c362c-766b-4eee-8012-034cd5c3960e)

---

## Weevil Portal

Symbiota portal dedicated to weevil data. Anyone can access the Weevil Portal, however you will need to login to download data and generate resources (e.g. checklists). Click [**here**](https://weevil.symbiota.org/portal/profile/newprofile.php) to create an account.


[<img src="https://github.com/user-attachments/assets/d4c552df-16b5-47ff-8fbb-62993662defc" alt="Weevil Portal">](https://weevil.symbiota.org)


## Anthribidae Catalog

Virtual catalog generated with TaxonWorks and TaxonPages.

[<img src="https://github.com/user-attachments/assets/96e328ad-93dc-4ca4-9aa2-dff0790df093" alt="Anthribidae TP">](https://anthribidae.github.io/species)


